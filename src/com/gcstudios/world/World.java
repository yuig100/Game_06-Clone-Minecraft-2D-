...
public class World {
	...
	public static int dia=0,tarde=1,noite=2;
	public static int CICLO = 0;
	
	public World(){
		String[] tilesTypes = {"terra","areia","neve"};
		WIDTH = 300;
		HEIGHT = 80;
		
		int divisao = (int)(WIDTH / tilesTypes.length);
				...
				if(yy == (HEIGHT) - 1 || xx == (WIDTH) - 1 || xx==0|| yy==0) {
					...
					if(yy >=inicialHeight) {
						...
						if(tilesTypes[indexBioma] == "terra") {
							...
							tiles[xx+yy*WIDTH] = new WallTile(xx*16,yy*16,Tile.TILE_TERRA_NEVE);
